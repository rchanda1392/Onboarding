...
```mermaid
graph TB
    CS["Cloud Services Layer<br/>Query optimization, security, metadata"]
    QP["Compute Layer<br/>Virtual Warehouse 1 · Virtual Warehouse 2 · Virtual Warehouse N"]
    ST["Storage Layer<br/>Micro-partitions, columnar, compressed"]
    CS --> QP --> ST
    Note["Compute and Storage<br/>scale independently"]
    QP -.- Note
    ST -.- Note
```
...
```mermaid
graph TB
    RD["Raw Data<br/>Data Lake — any format, any scale"]
    DL["Delta Lake<br/>ACID transactions, versioning, schema enforcement"]
    UC["Unity Catalog<br/>Governance, lineage, access control"]
    WL["Analytics & ML Workloads<br/>Databricks SQL · MLflow · Notebooks"]
    RD --> DL --> UC --> WL
    Note["Lakehouse = Data Lake flexibility<br/>+ Warehouse reliability"]
    DL -.- Note
```
...
```mermaid
mindmap
  root((Observability<br/>Tools Ecosystem))
