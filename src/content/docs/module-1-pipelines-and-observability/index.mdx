...
graph LR
    subgraph ETL["ETL (Traditional)"]
        direction LR
        A1[Sources] --> B1[Extract] --> C1[Transform<br/>in staging] --> D1[Load] --> E1[Data<br/>Warehouse]
    end
    subgraph ELT["ELT (Modern)"]
        direction LR
        A2[Sources] --> B2[Extract] --> D2[Load raw] --> E2[Data<br/>Warehouse] --> C2[Transform<br/>in place]
    end
```
...
graph LR
    subgraph Batch["Batch Processing"]
        direction LR
        BA[Data<br/>accumulates] --> BB[Scheduled<br/>trigger] --> BC[Process<br/>batch] --> BD[Output]
    end
    subgraph Stream["Streaming (Real-time)"]
        direction LR
        SA[Events<br/>arrive] --> SB[Process<br/>immediately] --> SC[Output<br/>continuously]
    end
```
...
```mermaid
graph TB
    SLA["SLA — Business Promise<br/>99.9% data freshness guaranteed"]
    SLO["SLO — Internal Target<br/>Data updated within 15 min, 99.95%"]
    SLI["SLI — Measured Metric<br/>Actual update latency = 12 min"]
    SLA --> SLO --> SLI
```
...
graph LR
    ...
    Software --- Common["Shared goal:<br/>Detect silent failures<br/>before users notice"]
    Data --- Common
```
