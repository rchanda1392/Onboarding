...
graph TB
    subgraph Training["Training Pipeline"]
        direction LR
        TD["Raw\nData"] --> TF["Feature\nComputation A"] --> TM["Model\nTraining"]
    end
    subgraph Serving["Serving Pipeline"]
        direction LR
        LD["Live\nData"] --> SF["Feature\nComputation B"] --> SI["Model\nInference"]
    end
    TF -.- SKEW["SKEW: These must\ncompute identically!"]
    SF -.- SKEW
```
...
graph TD
    Q1{"Large Language\nModel?"}
    Q1 -->|Yes| TPU1["TPU — optimized for\nGoogle's frameworks"]
    Q1 -->|No| Q2{"Need multi-framework\nflexibility?"}
    Q2 -->|Yes| GPU1["GPU — CUDA ecosystem\nuniversal support"]
    Q2 -->|No| Q3{"Google Cloud\necosystem?"}
    Q3 -->|Yes| TPU2["TPU — native\nintegration"]
    Q3 -->|No| GPU2["GPU — multi-cloud\ncompatible"]
```
