---
title: "Module 4: ML/AI Infrastructure — From Data to Model"
...
```mermaid
graph LR
    PF["Problem<br/>Framing"] --> DE["Data<br/>Exploration"] --> FE["Feature<br/>Engineering"]
    FE --> EX["Experimentation"] --> EV["Evaluation"] --> PR["Productionization"]
    subgraph eighty["~80% of time spent here"]
        PF
        DE
        FE
    end
    subgraph twenty["~20% — where value is delivered"]
        EX
        EV
        PR
    end
...
Raw data isn't directly useful to models. The researcher transforms it into **features** — signals the model can learn from. This might mean computing "average session duration over the last 7 days" from raw clickstream logs, or extracting text embeddings from product descriptions.

### Step 4: Experimentation

Now comes the actual model building — but it's not a single attempt. It's dozens or hundreds of experiments: trying different model architectures, hyperparameters, feature combinations, and training strategies. Each experiment is tracked, compared, and analyzed.
...
Once a model performs well offline, it needs to work in production — at scale, with low latency, handling real traffic. This is often called the "last mile" and it's where many ML projects die. The researcher's Jupyter notebook that works on a sample dataset needs to become a robust service handling millions of requests per second.
...
> **Key Insight:** Most of an ML researcher's time isn't spent on modeling — estimates suggest **80% of ML work is data preparation, cleaning, and feature engineering.** As a PM, if you only plan for the "model building" part, your timelines will be wildly off.

### The "Last Mile" Problem

The gap between a working prototype and a production system is enormous. A model that runs in a Jupyter notebook on a laptop is fundamentally different from one that serves 100,000 predictions per second with 50ms latency. This gap is why "MLOps" exists — it's the discipline of making ML actually work in production, reliably, at scale.

Think of it like the difference between cooking a great meal at home and running a restaurant. The recipe is the same, but everything around it — supply chain, consistency, speed, scale — is a completely different problem.

### Resources

- 📄 [Rules of ML: Best Practices for ML Engineering — Martin Zinkevich (Google)](https://developers.google.com/machine-learning/guides/rules-of-ml) — The definitive guide to practical ML engineering from Google, written for practitioners and PMs
- 📄 [Software 2.0 — Andrej Karpathy](https://karpathy.medium.com/software-2-0-a64152b37c35) — Influential essay on how ML changes software development fundamentals
- 📚 [Designing Machine Learning Systems — Chip Huyen (O'Reilly)](https://www.oreilly.com/library/view/designing-machine-learning/9781098107956/) — Comprehensive guide covering the full ML lifecycle from problem framing to production

---

## Section 2: Data Collection & Labeling

Every ML model starts with data — and the quality of that data is the single biggest determinant of model quality. "Garbage in, garbage out" is the oldest cliche in data science, but it's true at every scale.
...
mindmap
  root((Training Data<br/>Sources))
    ...
    Active Learning
      Smart sampling, cost-efficient
    Synthetic Data
      Generated, unlimited scale
```

### Data Labeling Workflows

**Supervised learning** — the most common type of ML — requires labeled data. Someone (or something) has to look at each example and tag it with the correct answer. This is labeling, and it's one of the most expensive and time-consuming parts of ML.

**Human annotation** is the gold standard. Expert annotators review examples and assign labels according to detailed guidelines. This produces high-quality labels but is slow and expensive — especially for tasks requiring domain expertise (like medical imaging or legal document review).

**Crowd-sourcing** platforms like Scale AI, Labelbox, and Amazon Mechanical Turk distribute labeling work across many workers. This scales better but introduces quality challenges — you need redundancy (multiple annotators per example), quality checks, and careful guideline design.
...
> **Key Insight:** Data labeling is often the bottleneck for ML projects — not model architecture, not compute, not engineering talent. When an ML team says they need "more data," they usually mean "more labeled data," and that means human time and money. As a PM, understanding this bottleneck is critical for realistic timeline planning.

### Resources

- 📄 [A Guide to Data Labeling for AI — Scale AI](https://scale.com/guides/data-labeling-annotation-guide) — Comprehensive guide from a leading data labeling platform
- 📄 [Active Learning: A Survey — Settles (2009)](https://burrsettles.com/pub/settles.activelearning.pdf) — The foundational academic survey on active learning
- 📄 [The Data-Centric AI Movement — Andrew Ng](https://datacentricai.org/) — Andrew Ng's initiative arguing that improving data quality matters more than model architecture
- 📄 [Synthetic Data for Machine Learning — Google Cloud](https://cloud.google.com/blog/products/ai-machine-learning/synthetic-data-for-machine-learning) — How Google Cloud approaches synthetic data generation
...
- `transaction_amount` — the dollar value of the purchase
- `avg_transaction_last_30d` — the user's average transaction amount over the past 30 days
- `time_since_last_transaction` — how many minutes since the user's previous purchase
- `country_mismatch` — whether the transaction country differs from the user's home country
- `device_fingerprint_new` — whether this device has been seen before
...
| **Embeddings** | Convert text/images to dense numeric vectors | Product description → 128-dimensional vector |
| **Cross-features** | Combine two features into one | `user_country × product_category` interaction |
...
| **Encoding** | Convert categories to numbers | Country → one-hot vector or ordinal encoding |
...
graph TB
    subgraph Training["Training Pipeline"]
        direction LR
        TD["Raw<br/>Data"] --> TF["Feature<br/>Computation A"] --> TM["Model<br/>Training"]
    end
    subgraph Serving["Serving Pipeline"]
        direction LR
        LD["Live<br/>Data"] --> SF["Feature<br/>Computation B"] --> SI["Model<br/>Inference"]
    end
    TF -.- SKEW["SKEW: These must<br/>compute identically!"]
    SF -.- SKEW
```

### The Feature Store Concept

A **feature store** solves the training-serving skew problem by providing a centralized repository for feature definitions and computed values.

```mermaid
graph TB
    RD["Raw Data"] --> FP["Feature<br/>Pipelines"]
    FP --> FS["Feature Store"]
    FS --> ON["Online Store<br/>Low-latency serving"]
    FS --> OF["Offline Store<br/>Batch training"]
    ON --> SP["Serving<br/>Pipeline"]
    OF --> TP["Training<br/>Pipeline"]
```

As ML teams grow, a recurring problem emerges: different teams compute the same features differently. The fraud team calculates "average transaction amount" one way, the recommendations team calculates it another way. Neither knows the other exists. When the same feature is used for training and serving, slight differences in computation lead to **training-serving skew** — the model sees different data in production than it saw during training, and performance silently degrades.

A **feature store** solves this by providing a centralized repository for feature definitions and computed feature values. Think of it as a shared library for ML features — compute once, use everywhere, guarantee consistency.
...
> **Key Insight:** Feature stores solve one of ML's sneakiest bugs — **training-serving skew**. If the features your model trains on are computed differently from the features it sees in production, performance silently degrades. It's like studying for a test using the wrong edition of the textbook — the material looks similar, but the details are different enough to trip you up.

### Resources

- 📄 [What is a Feature Store? — Tecton](https://www.tecton.ai/blog/what-is-a-feature-store/) — Clear introduction to the feature store concept and why it matters
- 📄 [Feast: An Open Source Feature Store — Feast Documentation](https://docs.feast.dev/) — Official docs for the most popular open-source feature store
- 📄 [Vertex AI Feature Store Documentation — Google Cloud](https://cloud.google.com/vertex-ai/docs/featurestore/overview) — Google's managed feature store offering
- 📄 [Feature Engineering for Machine Learning — O'Reilly](https://www.oreilly.com/library/view/feature-engineering-for/9781491953235/) — Deep dive into feature engineering techniques and best practices
...
Modern ML models can have billions of parameters (GPT-4 reportedly has over 1 trillion). Training them requires processing petabytes of data through these parameters, adjusting weights based on errors, and repeating the process for thousands of iterations. A single machine — even with a powerful GPU — would take months or years.
...
```mermaid
graph TD
    Q1{"Large Language<br/>Model?"}
    Q1 -->|Yes| TPU1["TPU — optimized for<br/>Google's frameworks"]
    Q1 -->|No| Q2{"Need multi-framework<br/>flexibility?"}
    Q2 -->|Yes| GPU1["GPU — CUDA ecosystem<br/>universal support"]
    Q2 -->|No| Q3{"Google Cloud<br/>ecosystem?"}
    Q3 -->|Yes| TPU2["TPU — native<br/>integration"]
    Q3 -->|No| GPU2["GPU — multi-cloud<br/>compatible"]
```
...
Google's TPU pods — clusters of thousands of TPU chips connected by high-speed interconnects — can train models in hours that would take GPU clusters days. This is a meaningful competitive advantage.

### Experiment Tracking

An ML researcher might run hundreds of experiments while developing a model. Each experiment has different hyperparameters (learning rate, batch size, model architecture choices), different training data subsets, and different feature combinations. Without tracking, it's impossible to know which configuration produced the best results — or to reproduce them later.
...
Hyperparameters are the configuration knobs of ML training — learning rate, batch size, number of layers, dropout rate, etc. Finding the optimal combination is critical but expensive.

- **Grid search**: Try every combination in a predefined grid. Thorough but wasteful — most combinations are useless.
- **Random search**: Sample random combinations. Surprisingly effective — [research shows](https://www.jmlr.org/papers/v13/bergstra12a.html) it finds good configurations faster than grid search in most cases.
...
Training a model isn't a single step — it's a pipeline: ingest data → validate → transform → split → train → evaluate → register. ML pipeline tools orchestrate this workflow:
...
Without all of these, reproducing a result — even your own from last week — becomes a guessing game.

> **Key Insight:** Training infrastructure is one of the most expensive line items in any ML organization's budget. A single large model training run on a TPU pod can cost hundreds of thousands of dollars. As a PM, understanding compute costs helps you make realistic prioritization decisions — is this experiment worth the compute budget?

### Resources

- 📄 [An Introduction to Distributed Training — Hugging Face](https://huggingface.co/docs/transformers/perf_train_gpu_many) — Practical guide to distributed training concepts and techniques
- 📄 [Google Cloud TPU Documentation](https://cloud.google.com/tpu/docs/intro-to-tpu) — Introduction to TPUs and how they compare to GPUs
- 📄 [MLflow Documentation](https://mlflow.org/docs/latest/index.html) — Official docs for the most popular open-source experiment tracking platform
- 📄 [TFX: A TensorFlow-Based Production-Scale ML Platform — Google (KDD 2017)](https://research.google/pubs/tfx-a-tensorflow-based-production-scale-machine-learning-platform/) — The original TFX paper explaining Google's ML pipeline architecture
- 📄 [Random Search for Hyper-Parameter Optimization — Bergstra & Bengio (2012)](https://www.jmlr.org/papers/v13/bergstra12a.html) — The influential paper showing random search beats grid search

---

## Section 5: Model Serving & Monitoring

A trained model sitting in storage is just a file. To create value, it needs to serve predictions to users, applications, and other systems — reliably, at scale, and with low latency.

```mermaid
graph LR
    TR["Train"] --> VA["Validate"] --> DP["Deploy"] --> MO["Monitor"]
    MO --> DD{"Detect<br/>Drift?"}
    DD -->|Data Drift| RT["Retrain"]
    DD -->|Concept Drift| RT
    DD -->|Feature Drift| RT
    DD -->|No| MO
    RT --> TR
...
A **model registry** is a versioned store for trained models — think of it like Git, but for ML models. It tracks:
...
Production ML creates a virtuous cycle: the model serves predictions → users interact with those predictions → those interactions become new training data → the next model version improves.
...
> **Key Insight:** Deploying a model isn't the finish line — it's the starting line. Models degrade over time as the world changes. The infrastructure for monitoring and retraining is just as important as the infrastructure for training in the first place. Think of it less like shipping software and more like tending a garden — it needs ongoing care.

### Resources

- 📄 [ML Model Monitoring — Google Cloud](https://cloud.google.com/vertex-ai/docs/model-monitoring/overview) — Google's approach to model monitoring and drift detection
- 📄 [Monitoring Machine Learning Models in Production — Evidently AI](https://www.evidentlyai.com/blog/machine-learning-monitoring-guide) — Practical guide to production ML monitoring
- 📄 [Hidden Technical Debt in Machine Learning Systems — Google (NeurIPS 2015)](https://papers.nips.cc/paper/2015/hash/86df7dcfd896fcaf2674f757a2463eba-Abstract.html) — Landmark paper on the maintenance burden of production ML systems
- 📄 [Continuous Delivery for Machine Learning — ThoughtWorks](https://martinfowler.com/articles/cd4ml.html) — How to apply CD practices to ML model deployment

---

## Key Takeaways

- **80% of ML work is data** — not modeling. Data collection, labeling, cleaning, and feature engineering dominate the timeline and budget. Plan accordingly.
- **The ML researcher's workflow is iterative** — problem framing → data exploration → feature engineering → experimentation → evaluation → productionization. Many cycles, many failures. This is normal, not a sign of poor planning.
...
- **Feature stores exist to prevent training-serving skew** — one of the sneakiest bugs in production ML. If features are computed differently during training and serving, the model silently underperforms.
- **Training at scale is expensive** — GPU/TPU costs can dominate ML budgets. Understanding compute costs helps you prioritize experiments and make ROI-based decisions.
- **Experiment tracking is non-negotiable** — without it, results are irreproducible and team collaboration breaks down.
- **Models degrade over time** — data drift, concept drift, and feature drift all require monitoring and retraining infrastructure. Shipping a model without monitoring is like launching a satellite without ground control.
- **The feedback loop is both powerful and dangerous** — production predictions influence future training data, which can amplify biases if not carefully monitored.

---

## Reflect & Apply

1. **Data bottleneck**: If your team wanted to build an ML feature that detects anomalies in data pipeline behavior, where would the labeled training data come from? How would you define what counts as an "anomaly" — and who would create those labels?
